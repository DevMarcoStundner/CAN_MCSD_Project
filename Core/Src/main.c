...
uint8_t TxData[8] = {0,10,2,3,4,5,6,7};			// Send data area
uint8_t Flag_Rx = 0;							// The logo of receiving data
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if(Flag_Rx)
	  {
		  Flag_Rx = 0;
		  /*
		   * Data mit uart an pc ausgeben
		   */
	  }

	  if(HAL_CAN_AddTxMessage(&hcan1, &CanTx, TxData, &pTxMailbox) != HAL_OK)		//send data
	  {
		  // uart ausgabe mit fehler
	  }
	  CanTx.ExtId++;
	  HAL_Delay(500);
    /* USER CODE BEGIN 3 */
